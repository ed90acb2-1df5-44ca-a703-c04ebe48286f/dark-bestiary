...
namespace DarkBestiary.Data
{
    public static class DataCollector
    {
        public static void Collect()
        {
            ...
            {
                File.WriteAllText(Environment.s_StreamingAssetsPath + "/compiled/data/" + file.Key, file.Value);
            }

            foreach (var dictionary in ConcatI18N())
            {
                File.WriteAllText(
                    Environment.s_StreamingAssetsPath + "/compiled/i18n/" + dictionary.Value.Name + ".json",
        ...
        {
            foreach (var file in Directory.GetFiles(Environment.s_StreamingAssetsPath + "/compiled/i18n", "*.json"))
            {
                File.Delete(file);
            }

            var core = Directory.GetFiles(Environment.s_StreamingAssetsPath + "/core/i18n", "*.json")
                .Select(file => JsonConvert.DeserializeObject<I18NDictionary>(File.ReadAllText(file)))
                .ToDictionary(dictionary => dictionary.Name);

            foreach (var folder in Directory.GetDirectories(Environment.s_StreamingAssetsPath + "/mods/"))
            {
        ...
        {
            var modpath = Environment.s_StreamingAssetsPath + "/mods/";
        ...
        {
            var core = Directory.GetFiles(Environment.s_StreamingAssetsPath + "/core/data", "*.json").ToDictionary(Path.GetFileName, ToDictionary);
