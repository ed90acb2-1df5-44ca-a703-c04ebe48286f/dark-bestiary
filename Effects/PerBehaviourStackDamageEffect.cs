...
namespace DarkBestiary.Effects
{
    public class PerBehaviourStackDamageEffect : DamageEffect
    {
        ...
        public PerBehaviourStackDamageEffect(PerBehaviourStackDamageEffectData data,
            List<Validator> validators, IEffectRepository effectRepository) : base(data, validators, effectRepository)
        {
            this.data = data;
        }
        ...
        protected override float Modify(float amount, GameObject caster, GameObject target)
        {
            ...
            return amount * behaviours.Behaviours.Where(b => (b.StatusFlags & this.data.StatusFlags) > 0)
                       .Sum(b => b.StackCount);
        }
    }
}